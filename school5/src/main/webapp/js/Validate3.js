var validateRegExp = {
    ...
    teacherCardId:"^[1-9]([0-9]{14}|[0-9]{17})$", //身份证
    ip4:"^(25[0-5]|2[0-4]\\d|[0-1]\\d{2}|[1-9]?\\d)\\.(25[0-5]|2[0-4]\\d|[0-1]\\d{2}|[1-9]?\\d)\\.(25[0-5]|2[0-4]\\d|[0-1]\\d{2}|[1-9]?\\d)\\.(25[0-5]|2[0-4]\\d|[0-1]\\d{2}|[1-9]?\\d)$", //ip地址
    letter:"^[A-Za-z]+$", //字母
    letter_l:"^[a-z]+$", //小写字母
    letter_u:"^[A-Z]+$", //大写字母
    teacherPhone:"^0?(13|15|18)[0-9]{9}$", //手机
    notempty:"^\\S+$", //非空
    ...
    nickName:"^[A-Za-z0-9_\\-\\u4e00-\\u9fa5]+$", //用户名
    deptname:"^[A-Za-z0-9_()（）\\-\\u4e00-\\u9fa5]+$", //单位名
    zipcode:"^\\d{6}$", //邮编
    teacherName:"^[A-Za-z\\u4e00-\\u9fa5]+$", // 真实姓名
...
var validateRules = {
	...
	isoldPassword:function (str) {
        return /^.*([\W_a-zA-z0-9-])+.*$/i.test(str);
    },
    ispassword:function (str) {
        return /^.*([\W_a-zA-z0-9-])+.*$/i.test(str);
    },
    isagainpassword:function (str1, str2) {
        return (str1 == str2);
    },
    ...
    isteacherPhone:function (str) {
        return new RegExp(validateRegExp.teacherPhone).test(str);
    },
isteacherCardId:function (str) {
        return new RegExp(validateRegExp.teacherCardId).test(str);
    },
    ...
    isteacherName:function (str) {
        return new RegExp(validateRegExp.teacherName).test(str);
    },
...
var validateFunction = {
	...
	oldPassword:function (option) {
        var str1 = option.value;
       
        var format = validateRules.isoldPassword(option.value);
    ...
    password:function (option) {
        var str1 = option.value;
        var str2 = $("#againpassword").val();
        var format = validateRules.ispassword(option.value);
        var length = validateRules.betweenLength(option.value, 6, 16);
        $("#passwordstrength").hide();
        if (!length && format) {
            validateSettings.error.run(option, option.prompts.error.badLength);
    ...
    againpassword:function (option) {
        var str1 = option.value;
        var str2 = $("#password").val();
        var length = validateRules.betweenLength(option.value, 6, 16);
        var format2 = validateRules.isagainpassword(str1, str2);
        var format1 = validateRules.ispassword(str1);
        if (!length) {
            validateSettings.error.run(option, option.prompts.error.badLength);
        } else {
            if (!format1) {
    ...
    passwordstrength:function () {
        ...
        var element = $("#passwordstrength");
        var value = $("#password").val();
        if (value.length >= 6 && validateRules.ispassword(value)) {
    ...
    showPassword:function (type) {
        var v1 = $("#password").val(), s1 = $("#password").attr("sta"), c1 = document.getElementById("password").className, t1 = $("#password").attr("tabindex");
        var v2 = $("#againpassword").val(), s2 = $("#againpassword").attr("sta"), c2 = document.getElementById("againpassword").className, t2 = $("#againpassword").attr("tabindex");
        var P1 = $("<input type='" + type + "' value='" + v1 + "' sta='" + s1 + "' class='" + c1 + "' id='password' name='password' tabindex='" + t1 + "'/>");
        $("#password").after(P1).remove();
        $("#password").bind("keyup",
            function () {
                validateFunction.passwordstrength();
            }).jdValidate(validatePrompt.password, validateFunction.password)
        var P2 = $("<input type='" + type + "' value='" + v2 + "' sta='" + s2 + "' class='" + c2 + "' id='againpassword' name='againpassword' tabindex='" + t2 + "'/>");
        $("#againpassword").after(P2).remove();
