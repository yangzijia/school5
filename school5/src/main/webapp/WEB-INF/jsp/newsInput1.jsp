  ...
  <body>
   <form id="editor_id" name="newsadd" method="post">
           ...
           <br/>
			...
			<br/>
			<input type="button" id="button" name="button" value="提交" tabindex="6" />
   </form>
    <br/>
        <span><font  color="red"><h1>注:录入新闻时，确认左上角的“HTML代码”选择按钮未选中。新闻录入完成时选中“HTML代码按钮”,生成HTML代码之后再提交。</h1>	</font></span>
        ...
        dataType:"json",          
                success : function(data) { 
                      if(data.Status=="Success"){
                                  msgShow('系统提示', '新闻录入成功！', 'warning');   
                                  window.location.href="newsInput1.html";
                      }else if(data.Status=="CannotAnalyzeData"){
                            $.messager.alert('提示信息','没有数据，请输入数据！');   
                            window.location.href="newsInput1.html";
                      }else if(data.Status=="Fail"){
                            $.messager.alert('提示信息','新闻录入失败！'); 
                            window.location.href="newsInput1.html";
                      }             
                }
          });
