	...
	var rows = [];
    	function getData(){
			$.ajax({
				...
				url:'showStudentUnRegistered.html',
				async :false,
				dataType:'json',
		 ...
		 $.messager.confirm("提示信息", "你确定要删除这"+rows.length+"条信息吗?", function (r) {
                             if (r) {
                                 ...
                                 for (var i = 0; i < rows.length; i++) {
                                 	...
                                 	var id = rows[i].id;
                                 	var administratoranduser = {"nickName":nickName,"id":id};
                                     ids.push(administratoranduser);
                                 }
                                var obj=ids;
...
$('#password_rec').click(function(){
  ...
  <body style="overflow:hidden;">
 	<table id="dg">   
		...
		<thead>
			<tr>
            	<th data-options="field:'id',editor:{type:'validatebox',options:{required:true}}" width="80">学生学号</th>
            	<th data-options="field:'studentName',editor:{type:'validatebox',options:{required:true}}" width="80">学生姓名</th>
			</tr>
		</thead>
       	</table>
