...
$(document).ready(function(){
	$('#accept_a').click(function(){
		...
		if($('#cc').combobox('getText')=="--请选择--"){
			...
			return false;}
		 $.messager.confirm("提示信息", "你确定要加这"+rows.length+"条权限到+"+$('#cc').combobox('getText')+"吗?", function (r) {
                             if (r) {
                                 var ids = [];
                                ...
                                var obj={"roleNameId":roleNameId,"rescIdList":ids};
                                var str=JSON.stringify(obj);
                               $.ajax({  
							        type:'Post',
							       	contentType:'application/json',
									url:'addRescToRole.html',
									data: str,
							        dateType:'json',  
							        success:function(data){ 
							            var obj = jQuery.parseJSON(data);
							      		if(obj.Status=="Success"){	
							            	...
							            	for (var i = 0; i < rows.length; i++) {
							            	 ...
							            	 $('#dg').datagrid('reload');
							            	$.messager.alert('提示信息','添加权限成功');
							            	
							            }else{
							            	$.messager.alert('提示信息','添加权限失败');
	...
	<body>
		...
		<thead>
			<tr>
				<th data-options="field:'name',editor:{type:'validatebox',options:{required:true}}" width="80">地址</th>
