...
$(document).ready(function(){
        ...
        columns:[[  
            ...
            {field:'opt',title:'查看',
            	formatter:function(value,row,incex){
            		var s = '<a href="leaderCheckRoll_AlertDetail.html?StudentNumber='+row.studentNumber+'&StudentName='+row.studentName+'">详细信息</a> ';  
            		return s;
            	},width:80
            }
            ]],
       ...
       function getData(){
    		...
    		//var obj={"yearClass":"","profession":"","classId":""};
    		//var str = JSON.stringify(obj);
			$.ajax({
				type:'Post',
				contentType:'application/json',
				//data:str,
				url:'getwebLeaderGetAllRollAlert.html',
				async :false,
				dataType:'json',
				success:function(data){
					if(data.Status=="Fail"){
					$.messager.alert('提示信息','没有不及格同学');
				}
					rows = data;
...
$(document).ready(function() {
    ...
    $('#bt').click(function(){
    	function getData(){
    		...
    		var obj={"college":college,"yearClass":yearClass,"profession":profession,"classId":classId};
    		var str = JSON.stringify(obj);
    		alert(str);
			$.ajax({
				type:'Post',
				contentType:'application/json',
				data:str,
				url:'getwebLeaderGetRollAlert.html',
				async :false,
				dataType:'json',
				success:function(data){
					if(data.Status=="Fail"){
					$.messager.alert('提示信息','该班级没有不及格同学');
				}
					rows = data;
