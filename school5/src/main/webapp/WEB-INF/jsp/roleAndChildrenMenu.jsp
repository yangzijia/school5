...
$(document).ready(function(){
	$('#accept_a').click(function(){
		...
		if($('#cc').combobox('getText')=="--请选择--"){
			...
			return false;}
		 $.messager.confirm("提示信息", "你确定要加这"+rows.length+"条子菜单到"+$('#cc').combobox('getText')+"吗?", function (r) {
                             if (r) {
                                 var ids = [];
                                ...
                                var obj={"roleNameId":roleNameId,"childrenMenuIdList":ids};
                                var str=JSON.stringify(obj);
                               $.ajax({  
							        type:'Post',
							       	contentType:'application/json',
									url:'addChildrenMenuToRole.html',
									data: str,
							        dateType:'json',  
							        success:function(data){ 
							            var obj = jQuery.parseJSON(data);
							      		if(obj.Status=="Success"){	
							            	...
							            	for (var i = 0; i < rows.length; i++) {
							            	 ...
							            	 $('#dg').datagrid('reload');
							            	$.messager.alert('提示信息','添加子菜单成功');
							            	
							            }else{
							            	$.messager.alert('提示信息','添加子菜单失败');
	...
	<body>
		...
		<thead>
			<tr>
				<th data-options="field:'childrenmenuName',editor:{type:'validatebox',options:{required:true}}" width="80">子菜单名</th>
				<th data-options="field:'url',editor:{type:'validatebox',options:{required:false}}" width="80">地址</th>
